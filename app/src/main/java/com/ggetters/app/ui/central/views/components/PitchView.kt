...
class PitchView @JvmOverloads constructor(
    ...
    var onPlayerDrop: ((String, PlayerAvailability) -> Unit)? = null
    ...
    private val positionedPlayers = mutableMapOf<String, PlayerAvailability>()
    ...
    private fun drawPositionedPlayers(canvas: Canvas) {
        positionedPlayers.forEach { (position, player) ->
            formationPositions[position]?.let { point ->
                ...
                canvas.drawText(
                    player.playerName.split(" ").last(), // Last name only
                    point.x,
    ...
    fun updatePlayerPosition(position: String, player: PlayerAvailability) {
