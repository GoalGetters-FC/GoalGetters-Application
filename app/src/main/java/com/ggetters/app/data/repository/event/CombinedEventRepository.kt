...
class CombinedEventRepository @Inject constructor(
    ...
    override suspend fun sync() {
        ...
        offline.getDirtyEvents(teamId).forEach { e ->
            runCatching { online.upsert(e) }.onSuccess { offline.markClean(e.id) }
        }

        // pull + merge (simple replace for now)
    ...
    override fun getEventsByType(teamId: String, category: Int) = offline.getEventsByType(teamId, category)

    override fun getEventsByCreator(creatorId: String) = offline.getEventsByCreator(creatorId)
